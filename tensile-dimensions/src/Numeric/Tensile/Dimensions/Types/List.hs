...
module Numeric.Tensile.Dimensions.Types.List (
  module Numeric.Tensile.Dimensions.Types.List ,
  S.Sort(..)
) where
...
pattern Cons x xs <- (patTL @f @xs -> PatCons x xs)
  where
    Cons = Numeric.Tensile.Dimensions.Types.List.cons
...
pattern Snoc sx x <- (unsnocTL @f @xs -> PatSnoc sx x)
  where
    Snoc = Numeric.Tensile.Dimensions.Types.List.snoc
...
pattern Reverse sx <- (unreverseTL @f @xs -> PatReverse sx)
  where
    Reverse = Numeric.Tensile.Dimensions.Types.List.reverse
